...
abstract class RemoteDeployTask extends DefaultTask {
    ...
    def deploy() {
        project.ssh.run {
            session(remote) {
                ...
                execute "/bin/rm -fr ${remoteWarDir}${project.war.archiveName}"
                // Need to execute zip command from the working Directory in order to get relative paths correct
                execute "cd ${workingDir}; /usr/bin/zip -r ${remoteWarDir}${project.war.archiveName} ."
                // Cleanup
                execute "/bin/rm -rf ${workingDir}"
            }
        }
    ...
    protected void setEnvironment(String env) {
        SelectEnvironmentTask.setEnvironment(project, this, env)
    }
}
